...
btn.addEventListener("click", function (e) {
  // 取消預設事件 不然每次刪除會跑到連結 # 然後網頁飛到最上面去(助教寫的)
  
  // 為什麼這個點擊事件要特別寫取消預設事件? 之前做的例題的點擊事件都不用寫e.preventDefault  
  e.preventDefault();
  if (input.value.trim() == "") {
    alert("項目名稱不得為空！");
    return;
...
function renderData() {
  ...
  data.forEach(function (item, index) {
    if (!item.checked) {
      // 計算待完成項目有幾個
      count += 1;
      if (toggleTab == "all" || toggleTab == "pending") {
        str += `<li>
